...
from optimizers import (
    ...
    GradientOptSetting,
    GradientOptimizer,
    LineSearchOptSetting,
    LineSearchOptimizer,
    QuasiNewtonOptSetting,
    QuasiNewtonOptimizer,
    TrustRegionOptSetting,
...
def sanity_check(
    K, A, B, Q, X0, discount=None, f=None, g=None, h=None, tol=1e-6, verbose=True
):
    ...
    if h is not None:
        ...
        if np.abs(H0_EE - H0_EE_true) > tol:
            raise ValueError(
                "Sanity check failed! Hessian quadform does not match true value,"
            )
...
if __name__ == "__main__":
    ...
    settings_dict_linesearch = {
        ...
        "newton": {
            "setting": LineSearchOptSetting(
                x0=vK0, step_method="newton", min_grad_norm=MIN_GRAD_NORM
            ),
            "optimizer": LineSearchOptimizer,
        },
        "bfgs": {
            "setting": QuasiNewtonOptSetting(
                x0=vK0, step_method="bfgs", min_grad_norm=MIN_GRAD_NORM
            ),
            "optimizer": QuasiNewtonOptimizer,
        },
        "dfp": {
            "setting": QuasiNewtonOptSetting(
                x0=vK0, step_method="dfp", min_grad_norm=MIN_GRAD_NORM
            ),
            "optimizer": QuasiNewtonOptimizer,
        },
        "sr1": {
            "setting": QuasiNewtonOptSetting(
                x0=vK0, step_method="sr1", min_grad_norm=MIN_GRAD_NORM
            ),
            "optimizer": QuasiNewtonOptimizer,
        },
    }
    ...
    settings_dict_trustregion = {
        "dogleg": {
            "setting": TrustRegionOptSetting(
                x0=vK0, step_method="dogleg", min_grad_norm=MIN_GRAD_NORM
            ),
            "optimizer": TrustRegionOptimizer,
        },
        "2d_subspace": {
            "setting": TrustRegionOptSetting(
                x0=vK0, step_method="2d_subspace", min_grad_norm=MIN_GRAD_NORM
            ),
            "optimizer": TrustRegionOptimizer,
        },
        "cg_steihaug": {
            "setting": TrustRegionOptSetting(
                x0=vK0, step_method="cg_steihaug", min_grad_norm=MIN_GRAD_NORM
            ),
            "optimizer": TrustRegionOptimizer,
        },
    }
    ...
    settings_dp = {
        "policy_iteration": {"optimizer": policy_iteration},
    ...
    for settings_dict, category in zip(settings_dict_list, categories):
        ...
        for method, method_dict in settings_dict.items():
            ...
            if category == "Dynamic programming":
                ...
                # Optimize
                print(method + " method")
                t_hist, x_hist, f_hist, g_hist, h_hist = optimizer(
                    K0, A, B, Q, X0, min_grad_norm=MIN_GRAD_NORM
                )
            else:
                # Make the optimizer
                ...
                # Optimize
                print(method + " method")
                t_hist, x_hist, f_hist, g_hist, h_hist = opt.optimize(
                    obj, hidden_data=[A, B, Q, X0]
                )
            ...
            results_dict[method] = {"setting": setting, "result": result}
    ...
    for category, results_dict in categories_results_dict.items():
        ...
        fig, ax = plot_multi(
            results_dict, x_opt=vec(K_are), f_opt=f_are, category=category
        )
    ...
    fig, ax = plot_multi(
        results_dict_elite, x_opt=vec(K_are), f_opt=f_are, category="Representative"
    )
